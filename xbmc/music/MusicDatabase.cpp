...
void CMusicDatabase::ExportKaraokeInfo(const std::string & outFile, bool asHTML)
{
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      if ( asHTML )
        outdoc = "<tr><td>" + songnum + "</td><td>" + (std::string)StringUtils::Join(song.artist, g_advancedSettings.m_musicItemSeparator) + "</td><td>" + song.strTitle + "</td></tr>\r\n";
      else
        outdoc = songnum + '\t' + (std::string)StringUtils::Join(song.artist, g_advancedSettings.m_musicItemSeparator) + '\t' + song.strTitle + '\t' + song.strFileName + "\r\n";
