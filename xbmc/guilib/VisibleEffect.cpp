...
void CAnimation::Create(const TiXmlElement *node, const CRect &rect, int context)
{
  ...
  else if (type == "hidden") m_type = ANIM_TYPE_HIDDEN;
  else if (type == "focus")  m_type = ANIM_TYPE_FOCUS;
  else if (type == "unfocus") m_type = ANIM_TYPE_UNFOCUS;
  else if (type == "windowopen")  m_type = ANIM_TYPE_WINDOW_OPEN;
  else if (type == "windowclose")  m_type = ANIM_TYPE_WINDOW_CLOSE;
  // sanity check
