...
#if 0
#define ONRES(name) \
  virtual void ##name(NPT_Result res, PLT_DeviceDataReference& device, void* userdata) \
  { static_cast<PLT_MediaControllerDelegate*>(userdata)->##name(res, device, userdata); }
  ONRES(OnStopResult)
  ONRES(OnSetPlayModeResult)
  ONRES(OnSetAVTransportURIResult)
  ONRES(OnSeekResult)
  ONRES(OnPreviousResult)
  ONRES(OnPlayResult)
  ONRES(OnPauseResult)
  ONRES(OnNextResult)
#undef ONRES
