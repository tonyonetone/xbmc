...
bool CVideoDatabase::GetItems(const std::string &strBaseDir, VIDEODB_CONTENT_TYPE mediaType, const std::string &itemType, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
  ...
  else if (itemType == "artists" && mediaType == VIDEODB_CONTENT_MUSICVIDEOS)
    return GetActorsNav(strBaseDir, items, mediaType, filter);
  else if (itemType == "albums" && mediaType == VIDEODB_CONTENT_MUSICVIDEOS)
...
std::string CVideoDatabase::GetItemById(const std::string &itemType, int id)
{
  ...
  else if (itemType == "actors" || itemType == "directors" || itemType == "artists")
...
bool CVideoDatabase::GetRandomMusicVideo(CFileItem* item, int& idSong, const std::string& strWhere)
{
  try
  {
    ...
    std::string strSQL = "select * from musicvideoview";
    if (!strWhere.empty())
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      if (images && !bSkip)
      {
        if (!singleFiles)
        {
          std::string strFileName((std::string)StringUtils::Join(movie.m_artist, g_advancedSettings.m_videoItemSeparator) + "." + movie.m_strTitle);
          if (movie.m_iYear > 0)
            strFileName += StringUtils::Format("_%i", movie.m_iYear);
          item.SetPath(GetSafeFile(moviesDir, strFileName) + ".avi");
        }
  ...
  try
  {
    ...
    while (movie)
    {
      ...
      else if (strnicmp(movie->Value(), MediaTypeMusicVideo, 10) == 0)
      {
        ...
        CFileItem item(info);
        bool useFolders = info.m_basePath.empty() ? LookupByFolders(item.GetPath()) : false;
        std::string filename = (std::string)StringUtils::Join(info.m_artist, g_advancedSettings.m_videoItemSeparator) + "." + info.m_strTitle;
        if (info.m_iYear > 0)
          filename += StringUtils::Format("_%i", info.m_iYear);
        CFileItem artItem(item);
