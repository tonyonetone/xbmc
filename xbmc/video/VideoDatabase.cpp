  ...
  try
  {
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
          ...
          g_passwordManager.IsDatabasePathUnlocked(movie.m_strPath, *CMediaSourceSettings::Get().GetSources("video")))
      {
        ...
        CStdString path = StringUtils::Format("%ld%s", movie.m_iDbId,strExt.c_str());
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
...
void CVideoDatabase::GetMusicVideosByAlbum(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strFileName = m_pDS->fv("files.strFilename").get_asString();
      CStdString strExt = URIUtils::GetExtension(strFileName);
      CStdString strDir = StringUtils::Format("3/2/%ld%s",m_pDS->fv("musicvideo.idMVideo").get_asInt(),strExt.c_str());
  ...
  try
  {
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
          ...
          g_passwordManager.IsDatabasePathUnlocked(musicvideo.m_strPath, *CMediaSourceSettings::Get().GetSources("video")))
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        CStdString path = StringUtils::Format("%ld%s", record->at(0).get_asInt(), strExt.c_str());
        itemUrl.AppendPath(path);
...
bool CVideoDatabase::GetRandomMusicVideo(CFileItem* item, int& idSong, const CStdString& strWhere)
{
  try
  {
    ...
    *item->GetVideoInfoTag() = GetDetailsForMusicVideo(m_pDS);
    ...
    CStdString strExt = URIUtils::GetExtension(strFileName);
    CStdString path = StringUtils::Format("videodb://musicvideos/titles/%ld%s",item->GetVideoInfoTag()->m_iDbId,strExt.c_str());
    item->SetPath(path);
...
void CVideoDatabase::GetEpisodesByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strFileName = m_pDS->fv("files.strFilename").get_asString();
      CStdString strExt = URIUtils::GetExtension(strFileName);
      CStdString path = StringUtils::Format("videodb://tvshows/titles/%ld/%ld/%ld%s",m_pDS->fv("episode.idShow").get_asInt(),m_pDS->fv(2).get_asInt(),m_pDS->fv(0).get_asInt(),strExt.c_str());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
      items.Add(pItem);
      m_pDS->next();
...
void CVideoDatabase::GetMoviesByPlot(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strFileName = m_pDS->fv("files.strFilename").get_asString();
      CStdString strExt = URIUtils::GetExtension(strFileName);
      CStdString path = StringUtils::Format("videodb://movies/titles/%ld%s", m_pDS->fv(0).get_asInt(),strExt.c_str());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
...
void CVideoDatabase::ExportSingleVideoToXML(const CStdString& inPath, bool images /*=false*/, bool overwrite/*=false*/, const CStdString &outPath /*=""*/)
{
  ...
  CStdString strPath = URIUtils::GetDirectory(inPath);
  if (strPath.empty())
	  return;
  ...
  else if (itemType.Equals("tvshows") || itemType.Equals("episodes") || itemType.Equals("recentlyaddedepisodes"))
  ...
  {
    CStdString nfoFile(URIUtils::AddFileToFolder(outDir, outNfo));
